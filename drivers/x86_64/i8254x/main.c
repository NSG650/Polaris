...
static err_t lwip_send_packet(struct netif *netif, struct pbuf *p) {
	uint8_t *complete =
		(uint8_t *)((uintptr_t)pmm_allocz(ALIGN_UP(p->tot_len, PAGE_SIZE)) +
					MEM_PHYS_OFFSET);
	...
	i8254x_tx_packet(&i8254x_dev,
					 (void *)((uintptr_t)complete - MEM_PHYS_OFFSET),
					 (uint16_t)p->tot_len);

	pmm_free((void *)((uintptr_t)complete - MEM_PHYS_OFFSET),
			 ALIGN_UP(p->tot_len, PAGE_SIZE));
	return ERR_OK;
}
...
uint64_t driver_entry(struct module *driver_module) {
	...
	netif_add(&nic_i8254x.lwip, NULL, &netmask, NULL, NULL, lwip_dummy_init,
			  tcpip_input);

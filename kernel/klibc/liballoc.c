...
#define ALIGN_INFO       \
	sizeof(ALIGN_TYPE) * \
	  16 ///< Alignment information is stored right before the pointer. This is
...
void *PREFIX(malloc)(size_t req_size) {
	...
	while (maj != NULL) {
		...
		if (maj->first == NULL) {
			...
			p =
			  (void *)((uintptr_t)(maj->first) + sizeof(struct liballoc_minor));
		...
		while (min != NULL) {
			...
			if (min->next == NULL) {
				...
				if (diff >= (size + sizeof(struct liballoc_minor))) {
					...
					p =
					  (void *)((uintptr_t)min + sizeof(struct liballoc_minor));
					ALIGN(p);

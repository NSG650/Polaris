...
static time rtc_get_time() {
    time ret_time;
    ret_time.hour = read(0x4);
    ret_time.minute = read(0x2);
    ret_time.second = read(0);
    printf("The time is %d:%d:%d\n", bcdtobin(ret_time.hour), bcdtobin(ret_time.minute), bcdtobin(ret_time.second));
    return ret_time;
}

static datetime_t rtc_get_date_time() {
...
uint64_t get_unix_timestamp(void) {
    ...
    t += (3600 * datetime.time.hour) + (60 * datetime.time.minute) + datetime.time.second;

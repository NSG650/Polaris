...
#define ALIGN_INFO       \
	sizeof(ALIGN_TYPE) * \
		16 ///< Alignment information is stored right before the pointer. This
		   ///< is the number of bytes of information stored there.
...
#if defined DEBUG || defined INFO
			printf("liballoc: ERROR: Possible 1-3 byte overrun for magic %x != "
				   "%x\n",
				   min->magic, LIBALLOC_MAGIC);
			FLUSH();
#endif
...
#if defined DEBUG || defined INFO
			printf("liballoc: ERROR: multiple PREFIX(free)() attempt on %x "
				   "from %x.\n",
...
#ifdef DEBUG
	printf("liballoc: %x PREFIX(free)( %x ): ", __builtin_return_address(0),
		   ptr);
	FLUSH();
#endif

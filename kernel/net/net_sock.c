...
static bool net_sock_bind(struct socket *_this,
						  struct f_description *description, void *addr,
						  socklen_t len) {
	(void)description;
...
static ssize_t net_sock_sendmsg(struct socket *_this,
	...
	if (this->sock.protocol == SOCK_STREAM) {
		...
		for (size_t i = 0; i < msg->msg_iovlen; i++) {
			ssize_t ret = lwip_send(this->lwip_fd, msg->msg_iov[i].iov_base,
...
static ssize_t net_sock_getsockopt(struct socket *_this,
								   struct f_description *description, int level,
...
static ssize_t net_sock_setsockopt(struct socket *_this,
								   struct f_description *description, int level,

...
#define HBH_LEN(hbh)                       \
	((((hbh->ext.hopbyhop.len + 1) << 3) - \
	  2)) /* len in bytes, minus nxthdr and len byte */
...
int pico_ipv6_router_compare(void *ka, void *kb);
...
int pico_ipv6_is_solnode_multicast(const uint8_t addr[PICO_SIZE_IP6],
								   struct pico_device *dev);
...
int pico_ipv6_frame_push(struct pico_stack *S, struct pico_frame *f,
						 struct pico_ip6 *src, struct pico_ip6 *dst,
						 uint8_t proto, int is_dad);
int pico_ipv6_route_add(struct pico_stack *S, struct pico_ip6 address,
						struct pico_ip6 netmask, struct pico_ip6 gateway,
						int metric, struct pico_ipv6_link *link);
int pico_ipv6_route_del(struct pico_stack *S, struct pico_ip6 address,
						struct pico_ip6 netmask, struct pico_ip6 gateway,
						int metric, struct pico_ipv6_link *link);
void pico_ipv6_unreachable(struct pico_stack *S, struct pico_frame *f,
						   uint8_t code);

struct pico_ipv6_link *pico_ipv6_link_add(struct pico_device *dev,
										  struct pico_ip6 address,
										  struct pico_ip6 netmask);
struct pico_ipv6_link *pico_ipv6_link_add_no_dad(struct pico_device *dev,
												 struct pico_ip6 address,
												 struct pico_ip6 netmask);
int pico_ipv6_link_del(struct pico_stack *S, struct pico_device *dev,
					   struct pico_ip6 address);
int pico_ipv6_cleanup_links(struct pico_stack *S, struct pico_device *dev);
struct pico_ipv6_link *pico_ipv6_link_istentative(struct pico_stack *S,
												  struct pico_ip6 *address);
struct pico_ipv6_link *pico_ipv6_link_get(struct pico_stack *S,
										  struct pico_ip6 *address);
struct pico_device *pico_ipv6_link_find(struct pico_stack *S,
										struct pico_ip6 *address);
struct pico_ip6 pico_ipv6_route_get_gateway(struct pico_stack *S,
											struct pico_ip6 *addr);
struct pico_ip6 *pico_ipv6_source_find(struct pico_stack *S,
									   const struct pico_ip6 *dst);
struct pico_device *pico_ipv6_source_dev_find(struct pico_stack *S,
											  const struct pico_ip6 *dst);
struct pico_ipv6_link *pico_ipv6_link_by_dev(struct pico_device *dev);
struct pico_ipv6_link *pico_ipv6_link_by_dev_next(struct pico_device *dev,
												  struct pico_ipv6_link *last);
...
struct pico_ipv6_link *pico_ipv6_prefix_configured(struct pico_stack *S,
												   struct pico_ip6 *prefix);
struct pico_ipv6_route *pico_ipv6_gateway_by_dev(struct pico_stack *S,
												 struct pico_device *dev);
struct pico_ipv6_route *
pico_ipv6_gateway_by_dev_next(struct pico_stack *S, struct pico_device *dev,
							  struct pico_ipv6_route *last);
...
void pico_ipv6_router_down(struct pico_stack *S,
						   const struct pico_ip6 *address);

int pico_ipv6_mcast_join(struct pico_stack *S, struct pico_ip6 *mcast_link,
						 struct pico_ip6 *mcast_group, uint8_t reference_count,
						 uint8_t filter_mode, struct pico_tree *_MCASTFilter);
int pico_ipv6_mcast_leave(struct pico_stack *S, struct pico_ip6 *mcast_link,
						  struct pico_ip6 *mcast_group, uint8_t reference_count,
						  uint8_t filter_mode, struct pico_tree *_MCASTFilter);

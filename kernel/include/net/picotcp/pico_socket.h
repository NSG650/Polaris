...
#ifdef __linux__
#define PICO_DEFAULT_SOCKETQ \
	(16 * 1024) /* Linux host, so we want full throttle */
#else
#define PICO_DEFAULT_SOCKETQ                                                  \
	(6 * 1024) /* seems like an acceptable default for small embedded systems \
				*/
#endif
...
struct pico_socket *
pico_socket_open(struct pico_stack *S, uint16_t net, uint16_t proto,
				 void (*wakeup)(uint16_t ev, struct pico_socket *s));
...
int pico_socket_sendto_extended(struct pico_socket *s, const void *buf,
								const int len, void *dst, uint16_t remote_port,
...
int pico_socket_getname(struct pico_socket *s, void *local_addr, uint16_t *port,
						uint16_t *proto);
int pico_socket_getpeername(struct pico_socket *s, void *remote_addr,
							uint16_t *port, uint16_t *proto);

int pico_socket_connect(struct pico_socket *s, const void *srv_addr,
						uint16_t remote_port);
...
struct pico_frame *pico_socket_frame_alloc(struct pico_socket *s,
										   struct pico_device *dev,
										   uint16_t len);
...
int pico_transport_process_in(struct pico_stack *S, struct pico_protocol *self,
							  struct pico_frame *f);
struct pico_socket *pico_socket_clone(struct pico_socket *facsimile);
int8_t pico_socket_add(struct pico_socket *s);
int pico_transport_error(struct pico_stack *S, struct pico_frame *f,
						 uint8_t proto, int code);

/* Socket loop */
int pico_sockets_loop(struct pico_stack *S, int loop_score);
struct pico_socket *pico_sockets_find(struct pico_stack *S, uint16_t local,
									  uint16_t remote);
/* Port check */
int pico_is_port_free(struct pico_stack *S, uint16_t proto, uint16_t port,
					  void *addr, void *net);

struct pico_sockport *pico_get_sockport(struct pico_stack *S, uint16_t proto,
										uint16_t port);
...
#define PICO_SOCKET_SETOPT_DIS(socket, index) \
	(socket->opt_flags &= (uint16_t)~(1 << index))
#define PICO_SOCKET_GETOPT(socket, index) \
	((socket->opt_flags & (1u << index)) != 0)

...
#define PICOTCP_MUTEX_UNLOCK(x) \
	do {                        \
	} while (0)
#define PICOTCP_MUTEX_DEL(x) \
	do {                     \
	} while (0)
#endif
...
static inline void
pico_queue_register_listener(struct pico_stack *S, struct pico_queue *q,
							 void (*fn)(struct pico_stack *, void *),
							 void *arg) {
	q->listener = fn;
	q->listener_arg = arg;
	q->listener_stack = S;
...
#else
#define pico_queue_register_listener(S, q, fn, arg) \
	do {                                            \
	} while (0)
#define pico_queue_wakeup(q) \
	do {                     \
	} while (0)
#endif

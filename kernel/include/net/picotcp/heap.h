...
#define DECLARE_HEAP(type, orderby)                                            \
	...
	static inline type *heap_get_element(struct heap_##type *heap,             \
										 uint32_t idx) {                       \
	...
	static inline int heap_insert(struct heap_##type *heap, type *el) {        \
		...
		while ((i > 1) && (half->orderby > el->orderby)) {                     \
			memcpy(heap_get_element(heap, i), heap_get_element(heap, i / 2),   \
				   sizeof(type));                                              \
			i /= 2;                                                            \
			half = heap_get_element(heap, i / 2);                              \
		}                                                                      \
	...
	static inline int heap_peek(struct heap_##type *heap, type *first) {       \
		...
		for (i = 1; (i * 2u) <= heap->n; i = child) {                          \
			...
			if ((child != heap->n) &&                                          \
				(right_child->orderby < left_child->orderby))                  \
				child++;                                                       \
			left_child = heap_get_element(heap, child);                        \
			if (last->orderby > left_child->orderby)                           \
				memcpy(heap_get_element(heap, i),                              \
					   heap_get_element(heap, child), sizeof(type));           \
			else                                                               \
				break;                                                         \
		}                                                                      \

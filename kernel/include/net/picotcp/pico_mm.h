...
/*
 * Zero-initialized malloc function, will reserve a memory segment of length
 * uint32_t len This memory will be quickly allocated in a slab of fixed size if
 * possible or less optimally in the heap for a small variable size The fixed
 * size of the slabs can be changed dynamically via a statistics engine
 */
 ...
 * Pages are unused if they no longer contain slabs or heap, and they have been
...
/*
 * This function fills up a struct with used and free slab and heap space in the
 * memory manager The user is responsible for resource managment
 */
void pico_mem_profile_collect_data(
	struct profiling_data *profiling_page_struct);

/*
 * This function prints the general structure of the memory manager
 * Printf in this function can be rerouted to send this data over a serial port,
 * or to write it away to memory
 */
void pico_mem_profile_scan_data(void);

/*
 * This function returns the total size that the manager has received from the
 * system This can give an indication of the total system resource commitment,
 * but keep in mind that there can be many free blocks in this "used" size
 * Together with pico_mem_profile_collect_data, this can give a good estimation
 * of the total resource commitment
 */
uint32_t pico_mem_profile_used_size(void);

/*
 * This function returns a pointer to page 0, the main memory manager
 * housekeeping (struct pico_mem_manager). This can be used to collect data
 * about the memory in user defined functions. Use with care!
 */

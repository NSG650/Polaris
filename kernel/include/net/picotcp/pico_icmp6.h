...
/* ND configuration */
#define PICO_ND_MAX_FRAMES_QUEUED \
	4 /* max frames queued while awaiting address resolution */
...
/* router flag set? */
#define IS_ROUTER(x) \
	(((long_be(x->msg.info.neigh_adv.rsor) & (PICO_ND_ROUTER)) >> 31) & 0x1)
/* solicited flag set? */
#define IS_SOLICITED(x) \
	(((long_be(x->msg.info.neigh_adv.rsor) & (PICO_ND_SOLICITED)) >> 30) & 0x1)
/* override flag set? */
#define IS_OVERRIDE(x) \
	(((long_be(x->msg.info.neigh_adv.rsor) & (PICO_ND_OVERRIDE)) >> 29) & 0x1)
...
int pico_icmp6_ping(struct pico_stack *S, char *dst, int count, int interval,
					int timeout, int size,
					void (*cb)(struct pico_icmp6_stats *),
					struct pico_device *dev);
int pico_icmp6_ping_abort(struct pico_stack *S, int id);

int pico_icmp6_neighbor_solicitation(struct pico_device *dev,
									 struct pico_ip6 *tgt, uint8_t type,
									 struct pico_ip6 *dst);
int pico_icmp6_neighbor_advertisement(struct pico_stack *S,
									  struct pico_frame *f,
									  struct pico_ip6 *target);
int pico_icmp6_router_solicitation(struct pico_device *dev,
...
int pico_icmp6_parameter_problem(struct pico_stack *S, struct pico_frame *f,
								 uint8_t problem, uint32_t ptr);
...
int pico_icmp6_router_advertisement(struct pico_device *dev,
									struct pico_ip6 *dst);

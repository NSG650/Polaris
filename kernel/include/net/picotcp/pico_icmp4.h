...
int pico_icmp4_ping(struct pico_stack *S, char *dst, int count, int interval,
...
int pico_socket_icmp4_sendto_check(struct pico_socket *s, void *buf, int len,
								   void *dst, uint16_t remote_port);
int pico_socket_icmp4_recvfrom(struct pico_socket *s, void *buf, int len,
...
int pico_icmp4_param_problem(struct pico_stack *S, struct pico_frame *f,
							 uint8_t code);

...
/*
 *  Looks up a context entry for a particular IPv6-address contained in 'addr'
 * and returns it. Returns NULL if no entry is found. (See RFC4944)
 */
...
/*
 *  Creates a new, or updates and existing, context entry for a certain IPv6
 * address. (See RFC4944)
 */
void ctx_update(struct pico_ip6 addr, uint8_t id, uint8_t size,
				pico_time lifetime, uint8_t flags, struct pico_device *dev);
...
struct pico_dev_6lowpan {
	 ...
	 *  @param dst  Link layer destination address of the device
	 * (IETF-endianness)
	 *
	 *  @return length of the frame that is transmitted on success, -1 on
	 * failure
	 */
	int (*send)(struct pico_device *dev, void *_buf, int len,
				union pico_ll_addr src, union pico_ll_addr dst);
};

/* Initialisation routine for 6LoWPAN specific devices */
int pico_dev_6lowpan_init(struct pico_stack *S, struct pico_dev_6lowpan *dev,
						  const char *name, uint8_t *mac,
						  enum pico_ll_mode ll_mode, uint16_t mtu,
						  uint8_t nomac,
						  int (*send)(struct pico_device *dev, void *_buf,
									  int len, union pico_ll_addr src,
...
struct pico_6lowpan_ll_protocol {
	...
	int32_t (*addr_from_net)(union pico_ll_addr *addr, struct pico_frame *f,
							 int32_t dest);
...
int32_t pico_6lowpan_ll_sendto_dev(struct pico_device *dev,
								   struct pico_frame *f);
int32_t pico_6lowpan_stack_recv(struct pico_device *dev, uint8_t *buffer,
								uint32_t len, union pico_ll_addr *src,
								union pico_ll_addr *dst);

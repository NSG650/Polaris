...
#define IS_BCAST(f) \
	((f->flags & PICO_FRAME_FLAG_BCAST) == PICO_FRAME_FLAG_BCAST)
...
struct pico_frame {
	...
	/*Priority. "best-effort" priority, the default value is 0. Priority can be
...
uint16_t pico_dualbuffer_checksum(void *b1, uint32_t len1, void *b2,
								  uint32_t len2);
